      ...
      >
        {listPaymentMethods.map(({ title, subtitle, image }, idx) => (
          <RadioGroup.Item
            value={idx.toString()}
            className={cn(
              "relative flex w-full cursor-pointer items-center justify-between rounded-lg px-6 py-4 text-start shadow-sm focus:outline-none",
              selectedValue === idx.toString()
                ? "bg-radio-hover"
                : "bg-radio/50 hover:bg-radio",
            )}
          >
            <div className="flex items-center">
              ...
              <div className="flex max-w-[250px] flex-col truncate text-sm sm:max-w-lg">
                ...
                <span className="block truncate text-muted-foreground sm:inline">
                  {t(`CheckoutPage.label.${subtitle}`, { default: subtitle })}
                </span>
              </div>
            </div>
